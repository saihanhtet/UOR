...
# Create bar chart for vaccine type distribution
ggplot(types_dist, aes(x = Var1, y = Freq)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = Freq), vjust = -0.5, size = 3.5) +
  labs(title = "Distribution of Number of Vaccine Types Used by Countries",
       x = "Number of Vaccine Types",
       y = "Number of Countries") +
  theme_minimal()  # Removed grid()

# Save the plot
ggsave("/Users/m2air/Documents/UOR/DataAnalysis/Assignment_1/answers/10/Distribution of Number of Vaccine Types Used by Countries.png", width = 10, height = 6)
